import * as React from "react";
import {
  StyleProp,
  ViewStyle,
  Pressable,
  StyleSheet,
  TouchableOpacity,
  View,
} from "react-native";
import { Image } from "expo-image";
import { StackNavigationProp } from "@react-navigation/stack";
import { useNavigation, ParamListBase } from "@react-navigation/native";
import { Border, Color } from "../GlobalStyles";

type BottomNavBar1Type = {
  style?: StyleProp<ViewStyle>;
};

const BottomNavBar1 = ({ style }: BottomNavBar1Type) => {
  const navigation = useNavigation<StackNavigationProp<ParamListBase>>();

  return (
    <View style={[styles.bottomNavBar, style]}>
      <TouchableOpacity
        style={styles.homeIcon}
        activeOpacity={0.2}
        onPress={() => navigation.navigate("StartingScreen1")}
      >
        <Image
          style={[styles.icon, styles.iconLayout]}
          contentFit="cover"
          source={require("../assets/vector5.png")}
        />
      </TouchableOpacity>
      <Pressable style={styles.enterNew} onPress={() => {}}>
        <View style={[styles.enterNewChild, styles.vectorIconLayout]} />
        <Image
          style={[styles.vectorIcon, styles.vectorIconLayout]}
          contentFit="cover"
          source={require("../assets/vector6.png")}
        />
      </Pressable>
      <Pressable style={styles.profileIcon} onPress={() => {}}>
        <Image
          style={[styles.icon, styles.iconLayout]}
          contentFit="cover"
          source={require("../assets/group.png")}
        />
      </Pressable>
    </View>
  );
};

const styles = StyleSheet.create({
  iconLayout: {
    maxHeight: "100%",
    overflow: "hidden",
    maxWidth: "100%",
  },
  vectorIconLayout: {
    borderRadius: Border.br_mini,
    position: "absolute",
  },
  icon: {
    width: "100%",
    height: "100%",
  },
  homeIcon: {
    left: "19.53%",
    top: "28.13%",
    right: "73.49%",
    bottom: "28.13%",
    width: "6.98%",
    height: "43.75%",
    position: "absolute",
  },
  enterNewChild: {
    top: "0%",
    right: "0%",
    bottom: "0%",
    left: "0%",
    backgroundColor: Color.colorBlack,
    width: "100%",
    height: "100%",
  },
  vectorIcon: {
    height: "50%",
    width: "54.17%",
    top: "23.96%",
    right: "23.96%",
    bottom: "26.04%",
    left: "21.88%",
    maxHeight: "100%",
    overflow: "hidden",
    maxWidth: "100%",
  },
  enterNew: {
    height: "75%",
    width: "11.16%",
    top: "12.5%",
    right: "44.88%",
    bottom: "12.5%",
    left: "43.95%",
    position: "absolute",
  },
  profileIcon: {
    left: "72.33%",
    top: "21.88%",
    right: "19.3%",
    bottom: "21.88%",
    width: "8.37%",
    height: "56.25%",
    position: "absolute",
  },
  bottomNavBar: {
    alignSelf: "stretch",
    borderTopLeftRadius: Border.br_19xl,
    borderTopRightRadius: Border.br_19xl,
    backgroundColor: Color.colorDarkslategray_100,
    shadowColor: "rgba(0, 0, 0, 0.25)",
    shadowOffset: {
      width: 0,
      height: 0,
    },
    shadowRadius: 12,
    elevation: 12,
    shadowOpacity: 1,
    height: 64,
  },
});

export default BottomNavBar1;
